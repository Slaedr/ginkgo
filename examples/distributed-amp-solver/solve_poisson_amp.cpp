...
/// Ginkgo MPI communicator
using comm_t = gko::experimental::mpi::communicator;

/// Index type for subdomain local indexing
using local_idx_t = int;

/// Index type for global indexing across all subdomains
using global_idx_t = long;

using DistVec = gko::experimental::distributed::Vector<double>;

using DistMtx =
    gko::experimental::distributed::Matrix<double, local_idx_t, global_idx_t>;

using Schwarz =
    gko::experimental::distributed::preconditioner::Schwarz<double,
                                                            local_idx_t>;
...
using namespace ex_dist_amp;


std::shared_ptr<const DistVec> global_rhs(
    std::shared_ptr<const gko::Executor> exec, comm_t comm,
    const gko::size_type global_n, const std::vector<double>& local_values)
{
    auto local_b = Vec::create_const(
        exec, gko::dim<2>{local_values.size(), 1},
        gko::make_const_array_view(exec->get_master(), local_values.size(),
                                   local_values.data()),
        1);
    return gko::share(DistVec::create_const(
        exec, comm, gko::dim<2>{global_n, 1}, std::move(local_b)));
}

std::shared_ptr<const DistVec> global_exact_solution(
    std::shared_ptr<const gko::Executor> exec, comm_t comm,
    const gko::size_type global_n, const std::vector<double>& local_values)
{
    auto local_x = Vec::create_const(
        exec, gko::dim<2>{local_values.size(), 1},
        gko::make_const_array_view(exec->get_master(), local_values.size(),
                                   local_values.data()),
        1);
    return gko::share(DistVec::create_const(
        exec, comm, gko::dim<2>{global_n, 1}, std::move(local_x)));
}
...
int main(int argc, char* argv[])
{
    ...
    if (do_print) {
        std::cout << "AMPLify Distributed GMRES + Schwarz(FGS) for the Poisson"
                     " equation on a stretched grid\n";
        print_config(cfg);
    ...
    const auto data =
        generate_problem_data<double, global_idx_t, local_idx_t>(comm, cfg);
    ...
    const auto b = global_rhs(exec, comm, global_n, data.local_rhs);
    const auto x_ref =
        global_exact_solution(exec, comm, global_n, data.local_exact_solution);
    ...
    results["config"] = {{"nx", cfg.nx},
                         {"ny", cfg.ny},
                         {"nz", cfg.nz},
                         {"mesh_stretch_ratio", cfg.mesh_stretch_ratio},
                         {"local_n", local_n},
                         {"global_n", global_n},
                         {"local_nnz", local_nnz},
                         {"global_nnz", global_nnz},
    ...
    if (do_print) {
        ...
        const std::string out = cfg.output_file_prefix + "stretch_poisson_" +
